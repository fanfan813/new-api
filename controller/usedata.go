...
func parseIntListQuery(c *gin.Context, key string) []int {
	rawValues := c.QueryArray(key)
	if single := c.Query(key); single != "" && len(rawValues) == 0 {
...
func GetYcTokenAnalytics(c *gin.Context) {
	startTimestamp, _ := strconv.ParseInt(c.Query("start_timestamp"), 10, 64)
	endTimestamp, _ := strconv.ParseInt(c.Query("end_timestamp"), 10, 64)
	limit, _ := strconv.Atoi(c.Query("limit"))
	timezoneOffset, _ := strconv.Atoi(c.Query("timezone_offset"))
	if timezoneOffset < -14*60 || timezoneOffset > 14*60 {
		timezoneOffset = 0
	}
	userIds := parseIntListQuery(c, "user_ids")
	tokenIds := parseIntListQuery(c, "token_ids")
	metric := c.Query("metric")

	dates, err := model.GetYcTokenAnalytics(startTimestamp, endTimestamp, userIds, tokenIds, limit, metric, timezoneOffset)
	if err != nil {
		common.ApiError(c, err)
		return
	}
