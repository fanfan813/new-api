...
func formatUserPromptLogLine(now time.Time, entry UserPromptLogEntry) string {
	return fmt.Sprintf(`%s.%03d token="%s" model="%s" reasoning_effort="%s" path="%s" question="%s"`,
		...
		escapeUserPromptLogValue(normalizeUserPromptMarkdownEscapes(entry.Question)),
	)
}

func escapeUserPromptLogValue(value string) string {
	// 该日志主要用于人工查看，普通反斜杠保持原样；控制字符仍需转义，
	// 以维持一行一条记录。
...
func normalizeUserPromptMarkdownEscapes(value string) string {
	var normalized strings.Builder
	normalized.Grow(len(value))
	for i := 0; i < len(value); i++ {
		if value[i] != '\\' || i+1 >= len(value) || value[i+1] != '_' {
			normalized.WriteByte(value[i])
			continue
		}

		// Windows 路径中的 "\_" 是目录分隔符而非 Markdown 转义。
		tokenStart := strings.LastIndexAny(value[:i], " \t\r\n") + 1
		tokenPrefix := value[tokenStart:i]
		if strings.Contains(tokenPrefix, `:\`) || strings.HasPrefix(tokenPrefix, `\\`) {
