...
func TestFormatUserPromptLogLine(t *testing.T) {
	...
	line := formatUserPromptLogLine(now, UserPromptLogEntry{
		...
		Path:            "/v1/responses",
		Question:        "第一行\n第二行\t带 \"quote\"，路径 C:\\work\\_cache，字段 model\\_rank",
	})

	assert.Equal(t, `2026-06-27 22:09:42.420 token="fanfan1" model="gpt-5.5" reasoning_effort="high" path="/v1/responses" question="第一行\n第二行\t带 \"quote\"，路径 C:\work\_cache，字段 model_rank"`, line)
}

func TestNormalizeUserPromptMarkdownEscapesPreservesPaths(t *testing.T) {
	tests := []struct {
		input string
		want  string
	}{
		{input: `model\_rank`, want: `model_rank`},
		{input: `C:\work\_cache\data.txt`, want: `C:\work\_cache\data.txt`},
		{input: `\\server\_share\data.txt`, want: `\\server\_share\data.txt`},
		{input: `正则 \d+`, want: `正则 \d+`},
	}

	for _, tt := range tests {
		assert.Equal(t, tt.want, normalizeUserPromptMarkdownEscapes(tt.input))
	}
}
