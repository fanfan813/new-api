...
func GetYcTokenAnalytics(startTime int64, endTime int64, userIds []int, tokenIds []int, limit int, metric string, timezoneOffsetMinutes int) (*YcTokenAnalyticsData, error) {
	if limit <= 0 || limit > 50 {
		limit = 10
	}

	items, err := getYcTokenAnalyticsItems(startTime, endTime, userIds, tokenIds, limit, metric)
	if err != nil {
		return nil, err
	}

	trend, err := getYcTokenAnalyticsTrend(startTime, endTime, userIds, tokenIds, items, timezoneOffsetMinutes)
	if err != nil {
		return nil, err
	}
...
func getYcTokenAnalyticsItems(startTime int64, endTime int64, userIds []int, tokenIds []int, limit int, metric string) ([]YcTokenAnalyticsItem, error) {
	orderBy := "quota DESC"
	if metric == "tokens" {
		orderBy = "token_used DESC"
	}
...
func getYcTokenAnalyticsTrend(startTime int64, endTime int64, userIds []int, tokenIds []int, items []YcTokenAnalyticsItem, timezoneOffsetMinutes int) ([]YcTokenAnalyticsTrendItem, error) {
	if len(items) == 0 {
	...
	// JavaScript getTimezoneOffset is UTC minus local time. Shifting before
	// truncation makes daily buckets start at the browser's local midnight.
	timezoneOffsetSeconds := int64(timezoneOffsetMinutes * 60)
	for _, row := range rows {
		bucket := ((row.CreatedAt-timezoneOffsetSeconds)/bucketSeconds)*bucketSeconds + timezoneOffsetSeconds
