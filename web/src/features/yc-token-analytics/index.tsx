...
type TokenDetailSortKey =
  ...
  | 'errors'
  | 'last_used'
type TokenDetailSort = {
  key: TokenDetailSortKey
  direction: 'asc' | 'desc'
}
...
function YcTokenSortableHeader(props: {
  label: string
  sortKey: TokenDetailSortKey
  sort: TokenDetailSort | null
  align?: 'left' | 'right'
  onSort: (key: TokenDetailSortKey) => void
}) {
  const isActive = props.sort?.key === props.sortKey
  let SortIcon = ChevronsUpDown
  if (isActive) {
    SortIcon = props.sort?.direction === 'asc' ? ArrowUp : ArrowDown
  }
...
export function YcTokenCharts() {
  ...
  const [topTokenLimit, setTopTokenLimit] = useState(50)
  ...
  const [detailSort, setDetailSort] = useState<TokenDetailSort | null>(null)
  ...
  const handleRangeChange = useCallback((value: TokenRangeValue) => {
    ...
    let range = getRollingDateRange(preset.days)
    if (value === 'today') {
      range = getTodayRange()
  ...
  const handleApplyFilters = useCallback(async () => {
    ...
    if (selectedRange === 'today') {
      ...
      timeRangeChanged =
        nextTimeRange.start_timestamp !== timeRange.start_timestamp ||
        nextTimeRange.end_timestamp !== timeRange.end_timestamp
      if (timeRangeChanged) {
    ...
    // A changed query key triggers the request; refetch only when all inputs
    // remain unchanged so one click never sends duplicate requests.
    if (!filtersDirty && !timeRangeChanged) {
      await refetch()
    }
  }, [
    draftTokenIds,
      ...
      tooltip: {
        mark: {
          content: [
            {
              key: t('Token Name'),
              value: (datum: { Token: string }) => datum.Token,
            },
            {
              key: metricLabel,
              value: (datum: { Usage: number }) =>
                chartMetric === 'quota'
                  ? formatQuota(Number(datum.Usage) || 0)
                  : formatTokenAmount(Number(datum.Usage) || 0, tokenUnit),
            },
        ...
        dimension: {
          content: [
            {
              key: (datum: { Token: string }) => datum.Token,
              value: (datum: { Usage: number }) => datum.Usage,
            },
          ],
  ...
  const handleDetailSort = useCallback((key: TokenDetailSortKey) => {
  ...
  const sortedDetailItems = useMemo(() => {
    const items = tokenAnalytics?.items ?? []
    if (!detailSort) return items

    return [...items].sort((left, right) => {
      let comparison = 0
      switch (detailSort.key) {
        case 'token':
          comparison = (left.token_name || `#${left.token_id}`).localeCompare(
            right.token_name || `#${right.token_id}`
          )
          break
        case 'username':
        ...
        case 'count':
          ...
          break
        case 'quota':
          comparison = left.quota - right.quota
          break
        case 'tokens':
          comparison = left.token_used - right.token_used
          break
        case 'errors':
          comparison = left.error_count - right.error_count
          break
        case 'last_used':
          comparison = left.last_used_at - right.last_used_at
          break
      }
      return detailSort.direction === 'asc' ? comparison : -comparison
    })
  }, [detailSort, tokenAnalytics?.items])
